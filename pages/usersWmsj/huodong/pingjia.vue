			...
			<view class='evaluate-con pad30'>
				<view class='goodsStyle acea-row row-between borRadius14'>
					...
					<view class='text acea-row row-between'>
						<view>
							<view class='name line2'>{{activityInfo.activityName}}</view>
							<view class='attr line1 font_sm'>主办：{{activityInfo.tuanduiName}}</view>
						</view>
						<view class='money'>
							<view style="float: left;">活动计划人数：{{activityInfo.nowNumber}}</view>
							<view style="float: left;margin-left: 40rpx;">实际参加人数：{{activityInfo.nowNumber}}</view>
							<view style="float: left;">服务时长：{{activityInfo.serveHours}}</view>
						</view>
					</view>
				</view>
				<view class='score borRadius14'>
					<view class='item acea-row row-middle' v-for="(item,indexw) in scoreList" :key="indexw">
						...
						<view class='starsList'>
							<text @click="stars(indexn, indexw)" v-for="(itemn, indexn) in item.stars" :key="indexn" class='iconfont' 
							:class="item.index >= indexn? 'icon-shitixing':'icon-kongxinxing'"></text>
						</view>
						<text class='evaluate'>{{item.index === -1 ? "" : item.index + 1 + "分"}}</text>
					</view>
					<view class='textarea'>
						<textarea placeholder='活动服务满足你的期待么？说说你的想法，分享给他们吧~' name="comment"
...
<script>
	import { getActivityInfo,activityComment} from '@/api/wmsj.js';
	...
	export default {
		...
		data() {
			return {
				...
				activityInfo: {activityName:'',tuanduiName:'',nowNumber:'',serveHours:'',activityImages:''},
		...
		onLoad(options) {
			if (!options.activityId ) return this.$util.Tips({
				title: '缺少参数'
			}, {
				tab: 3,
				url: 1
		...
		methods: {
			...
			getActivity: function() {
				...
				getActivityInfo(that.activityId).then(res => {
					that.$set(that, 'activityInfo', res.data);
				});
			},
			...
			formSubmit: function(e) {
				...
				value.productId = '';
				value.orderNo = '';
				value.unique = that.activityId;
				value.sku = '';
				uni.showLoading({
					title: "正在发布评价……"
				});
				activityComment(value).then(res => {
					uni.hideLoading();
					return that.$util.Tips({
						title: '感谢您的评价!',
						icon: 'success'
					}, '/pages/usersWmsj/huodong/myDiandan');
				}).catch(err => {
					uni.hideLoading();
					return that.$util.Tips({
						title: err
