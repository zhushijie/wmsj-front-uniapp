	...
	<view :data-theme="theme">
		<view class="CommissionRank">
			<view class="header">
				...
				<view class="rank" v-else>您目前暂无前50名排名</view>
			</view>
			<view class="wrapper">
				...
				<view class="list">
					<view class="item acea-row row-between-wrapper" v-for="(item,index) in rankList" :key="index">
						...
						<view class="people font_color">{{item.serviceNum}}小时</view>
					</view>
				</view>
...
<script>
	import { getserviceNumRank, serviceNumRankNumber } from '@/api/wmsj.js';
	...
	export default {
		...
		watch:{
			isLogin:{
				handler:function(newV,oldV){
					if(newV){
						this.getServiceNumRankList();
						this.getServiceNumRankNumber(this.type);
					}
				},
				deep:true
			}
		...
		methods: {
			...
			getServiceNumRankNumber(type) {
				serviceNumRankNumber({
					type: type
				}).then(res => {
					this.position = res.data;
				})
			},
			getServiceNumRankList: function() {
				...
				getserviceNumRank({
					...
					this.loadTitle = loadend ? '😕已显示全部信息' : '加载更多';
