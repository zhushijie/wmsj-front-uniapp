<template>
	<view :data-theme="theme">
		<view class='my-order'>
			...
			<view class="num-wrapper" style="display: none;">
			...
			<view class='header bg_color'>
				<view class='picTxt acea-row row-between-wrapper'>
					<view class='text'>
						...
						<view>消费订单：{{orderData.orderCount || 0}} 
						总消费：￥{{orderData.sumPrice ? Number(orderData.sumPrice).toFixed(2) : 0}}
						余额：{{userInfo.nowMoney && uid ? userInfo.nowMoney:0}}
						</view>
					</view>
					<view class='pictrue'>
		...
		methods: {
			...
			getOrderList: function() {
				...
				getOrderList({
					...
					that.loading = false;
					that.loadTitle = loadend ? "已显示全部信息" : '加载更多';
