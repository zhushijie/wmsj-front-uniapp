	...
	<view :data-theme="theme">
		<view class='order-submission'>
			...
			<view class="pad30">
				...
				<view class='wrapper borRadius14'>
					...
					<view class='item acea-row row-between-wrapper'
						v-if="!orderInfoVo.bargainId && !orderInfoVo.combinationId 
						&& !orderInfoVo.seckillId && productType==='normal'&&orderInfoVo.isIntegral">
						<view>积分抵扣</view>
						<view class='discount acea-row row-middle'>
							<view> 当前积分:
								<text class='num font_color'>{{orderInfoVo.userIntegral}}</text>
							</view>
							
							<view> 抵扣后剩余
								<text class='num font_color'> {{orderInfoVo.surplusIntegral }}</text>
							</view>
				...
				<view class='moneyList borRadius14'>
					<view class='item acea-row row-between-wrapper'>
						<view v-if="orderInfoVo.isIntegral">商品价值：</view>
						<view v-else>商品总价：</view>
			...
			<view class='footer acea-row row-between-wrapper'>
				<view v-if="!orderInfoVo.isIntegral" >合计:
					<text class='price_color'>￥{{orderInfoVo.payFee || 0}}</text>
				</view>
				<view v-if="orderInfoVo.isIntegral" >积分:
					<text class='price_color'>￥{{orderInfoVo.integralPrice}}</text>
				</view>
				
				<view v-if="orderInfoVo.isIntegral&&orderInfoVo.payFee==0" class='settlement' style='z-index:100' @tap="SubOrder">立即兑换</view>
				<view v-if="orderInfoVo.isIntegral&&orderInfoVo.payFee!=0" class='nosettlement' style='z-index:100'>积分不足</view>
				<view v-if="!orderInfoVo.isIntegral" class='settlement' style='z-index:100' @tap="SubOrder">立即结算</view>
...
<style lang="scss" scoped>
	...
	.order-submission .footer .nosettlement {
		font-size: 30rpx;
		color: #fff;
		width: 240rpx;
		height: 70rpx;
