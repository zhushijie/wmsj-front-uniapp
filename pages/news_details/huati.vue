<template>
	...
	<view class='page' :data-theme="theme">
		<view style="width: 750rpx;">
			<img style="width: 750rpx;" :src="huatiInfo.extra"></img>
			<view class='title'>{{huatiInfo.name}}</view>
		</view>

		<view class='newsDetail'>
			<view :class="fold ? 'synopsis fold' : 'synopsis unfold'">{{huatiInfo.description}}</view>
		...
		<view>
			<comment-eg :commentObj="{'objId':huatiInfo.id,'objType':'news'}" :visit="huatiInfo.visit"></comment-eg>
		</view>
	</view>
</template>
	...
	export default {
		...
		onShareAppMessage: function() {
			return {
				...
				imageUrl: this.huatiInfo.extra,
				desc: this.huatiInfo.description,
		...
		methods: {
			getArticleOne: function() {
				...
				getPCategoryDetails({
					...
					that.$set(that, 'huatiInfo', res.data);
					...
					// #endif
					//this.getArticlehuati(that.id)
				});
			},
			getArticlehuati: function(id) {
				let that = this;
				getArticlehuatiList(id, {
					id: that.id
				}).then(res => {
					let articleList = []
