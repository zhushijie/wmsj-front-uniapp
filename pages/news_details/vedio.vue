<template>
	...
	<view class='page' :data-theme="theme">
		 <view style="width: 750rpx;" v-if="showVideos">
			<video style="width: 750rpx;" id="myVideo" data-id="myVideo" :poster=articleInfo.imageInput :src="articleInfo.videoPath" object-fit="fill" autoplay=true controls></video>
		</view>
					
		<view class='newsDetail'>
			<view class='title'>{{articleInfo.title}}</view>
			<view class='synopsis'>{{articleInfo.synopsis}}</view>
			<view class='list acea-row row-middle'>
				<view class='label'>作者：{{articleInfo.author}}</view>
				<view class='item'></text>{{articleInfo.createTime}}</view>
				<view class='item'><text class='iconfont icon-liulan'></text>浏览：{{articleInfo.visit}}</view>
			</view>
			...
			<!-- #ifdef H5 -->
			<button class="bnt bg_color" hover-class='none' @click="listenerActionSheet" v-if="this.$wechat.isWeixin()">和好友一起分享</button>
			<!-- #endif -->
			<!-- #ifdef MP -->
		...
		<home></home>
		<view>
			<comment-eg :commentObj="{'objId':articleInfo.id,'objType':'news'}" :visit="articleInfo.visit" ></comment-eg>
		</view>
	</view>
</template>
