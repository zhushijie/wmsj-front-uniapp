				...
				<scroll-view scroll-y="true">
					...
					<view class="head pad30">
						<view class="user-card">
							<view class="user-info">
								...
								<view class="info">
									...
									<view class="name" v-if="uid">
										<view v-if="userInfo.realName">{{userInfo.realName}}</view>
										<view v-else>{{userInfo.nickname}}</view>
										<view class="vip" v-if="userInfo.vip">
											<view style="" class="vip-txt">{{userInfo.vipName || ''}} </view>
										</view>
										
										<view class="vip" v-if="userInfo.groupId=='2'"   @tap="openWmsj">
											<view style="" class="vip-txt">{{userInfo.groupName || ''}}</view>
										</view>
										<view class="vip" v-if="userInfo.groupId!='2'">
											<view class="vip-txt"  @tap="openWmsj">
												点击成为志愿者
											</view>
										</view>
									</view>
							...
							<view class="num-wrapper" >
								<view class="num-item">
									<text class="num">{{userInfo.serviceNum && uid ? userInfo.serviceNum:0}}</text>
									<view class="txt">服务时长</view>
								</view>
								<view class="num-item" @click="goMenuPage('/pages/users/user_integral/index')">
									<text class="num">{{userInfo.integral && uid ? userInfo.integral: 0}}</text>
									<view class="txt">活动积分</view>
								</view>
						...
						<view class="order-wrapper"  style="display: none;">
					...
					<view class="contenBox" id="pageIndex">
						...
						<view class="user-menus" style="margin-top: 20rpx;" @click.native="bindEdit('userMenus')">
							<view class="menu-title">文明实践</view>
							<view class="list-box">
								<block v-for="(item,index) in MyWmsjMenus" :key="index">
									<view class="item" @click="menusTapWmsj(item.url)">
										<u-badge style="position: absolute;right: 30rpx; " v-if="item.name=='消息通知'"
										 numberType="ellipsis" :type="recordCountType" max="99" :value="recordCount"></u-badge>
						...
						<!-- 会员菜单 -->
						<view class="user-menus" style="margin-top: 20rpx;" @click.native="bindEdit('userMenus')">
							<view class="menu-title">融媒商城</view>
							<view class="list-box">
...
<script>
	...
	import {getCount} from '@/api/record.js';
		...
		methods: {
			...
			getMyMenus: function() {
				...
				getMenuList().then(res => {
					...
					that.$set(that, 'MyWmsjMenus', res.data.routine_wmsj_my_banner);
