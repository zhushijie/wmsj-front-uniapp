	...
	<view :data-theme="theme">
		...
		<view class="product-con skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
							...
							<view class='wrapper mb30 borRadius14'>
								<view class='share acea-row row-between row-bottom'>
									<view class='x-money skeleton-rect flex align-baseline'>￥
										<text v-if="!productInfo.isIntegral" class='num font-44'>{{attr.productSelect.price}}</text>
										<text v-if="productInfo.isIntegral">{{productInfo.integralPrice}}积分</text>
			...
			<view class='footer acea-row row-between-wrapper'  >
				...
				<!-- #endif -->
				<block v-if="type === 'normal' && !productInfo.isIntegral">
				...
				<view class="bnt bntVideo acea-row skeleton-rect"
					v-if="attr.productSelect.stock > 0 && type === 'video'&& !productInfo.isIntegral">
					<form @submit="goBuy" report-submit="true"><button class="buy bnts" form-type="submit">立即购买</button>
					</form>
				</view>
				
				<block v-if="productInfo.isIntegral">
					<view @click="setCollect" class='item skeleton-rect'>
						<view class='iconfont icon-shoucang1' v-if="userCollect"></view>
						<view class='iconfont icon-shoucang' v-else></view>
						<view>收藏</view>
					</view>
					<view v-if="attr.productSelect.stock > 0 " class="bnt bntVideo acea-row skeleton-rect">
						<form @submit="goBuy" report-submit="true"><button class="buy bnts" form-type="submit">立即兑换</button>
						</form>
					</view>
					<view v-if="attr.productSelect.stock <= 0 " class="bnt bntVideo acea-row skeleton-rect">
						<form @submit="goBuy" report-submit="true"><button class="buy bnts" form-type="submit">兑换商品补货中</button>
						</form>
					</view>
				</block>
	...
	export default {
		...
		methods: {
			...
			goCat: function(num) {
				...
				//积分兑换不看属性 其他商品多规格需要选择
				if(!that.productInfo.isIntegral){
					//打开属性
					if (that.attrValue) {
