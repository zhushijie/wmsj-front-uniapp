	...
	<view>
		<view class="content">
			<block v-for="(item,index) in list" :key="index">
				<view class="detail" v-if="item.type==1">
					<view class="textarea" v-if="item.type==1">
						<u--textarea  v-model="list[index].content" placeholder="请输入内容"  count maxlength="500" height="140"  ></u--textarea>
					</view>
					<view class="btn-group">
						<view :data-index="index" @tap="delItem"  v-if="item.type!=1">
							<uni-icons type="trash" size="14"></uni-icons>
							<text>删除</text>
						</view>
						<view v-if="index>0" :data-index="index" @tap="arrowthinup" style="display: none;">
							<uni-icons type="arrowthinup" size="14"></uni-icons>
							<text>上移</text>
						</view>
						<view v-if="index<(list.length-1)" :data-index="index" @tap="arrowthindown" style="display: none;">
							<uni-icons type="arrowthindown" size="14"></uni-icons>
							<text>下移</text>
						</view>
					</view>
				</view>
				<view class="detailImg" v-if="item.type==2">
					<view class="image">
				...
				<view class="detailVideo" v-if="item.type==3">
		...
		<view class="shade" v-if="shade" @tap="closeShade">
			<view class="shade-content" @tap.stop>
				<view class="shade-content-item" @tap="addText" style="display: none;">
			...
			for(let i in this.list){
				if(this.list[i].content!=""){
					...
					if(this.list[i].type==2){
						content += `<div style="width:33%;padding-top: 10px">
							<img src="${this.list[i].content}" style="width: 33%" />
						</div>`;
						// 封面图
						if(this.form.image==""){
